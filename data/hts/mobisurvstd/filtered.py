...
def execute(context):
    ...
    print(df_trips["origin_departement_id"].value_counts())
    ...
    df_households_pd.to_csv(context.config("output_path") + "/df_households_pd.csv", index=False)
    df_persons_pd.to_csv(context.config("output_path") + "/df_persons_pd.csv", index=False)
    df_trips_pd.to_csv(context.config("output_path") + "/df_trips_pd.csv", index=False)
