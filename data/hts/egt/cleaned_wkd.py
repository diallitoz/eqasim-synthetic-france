...
def configure(context):
    context.stage("data.hts.egt.raw_wkd")
...
def execute(context):
    df_menages_sam, df_menages_dim, df_personnes_sam, df_personnes_dim, df_deplacements_sam, df_deplacements_dim = context.stage("data.hts.egt.raw_wkd")
    
    df_menages_sam = df_menages_sam.assign(__origin='sam')
    df_menages_dim = df_menages_dim.assign(__origin='dim')
    df_personnes_sam = df_personnes_sam.assign(__origin='sam')
    df_personnes_dim = df_personnes_dim.assign(__origin='dim')
    df_deplacements_sam = df_deplacements_sam.assign(__origin='sam')
    df_deplacements_dim = df_deplacements_dim.assign(__origin='dim')

    df_households = pd.concat([df_menages_sam, df_menages_dim], ignore_index=True)
    df_persons = pd.concat([df_personnes_sam, df_personnes_dim], ignore_index=True)
    df_trips = pd.concat([df_deplacements_sam, df_deplacements_dim], ignore_index=True)
    ...
    print(df_trips)
    print(df_trips[["egt_trip_id"]].value_counts()[:10])

    def report_duplicates(df, id_col, name, show_head=10):
        if id_col not in df.columns:
            print(f"[{name}] colonn {id_col} not found.")
            return None
        mask_valid = df[id_col].notna()
        dup_mask = df[mask_valid].duplicated(subset=[id_col], keep=False)
        dup_df = df[dup_mask].sort_values(by=id_col)
        total_dup_rows = dup_df.shape[0]
        unique_dup_ids = int(dup_df[id_col].nunique()) if total_dup_rows else 0
        print(f"[{name}] Total duplicated lines: {total_dup_rows} (Unique IDs duplicated: {unique_dup_ids})")
        if total_dup_rows:
            print(f"Examples  (top {show_head}):")
            print(dup_df[[id_col]].value_counts()[:show_head])
            print("Extrait :")
            print(dup_df.head(show_head))
        return dup_df

    dup_hh = report_duplicates(df_households, "egt_household_id", "households")
    dup_persons = report_duplicates(df_persons, "egt_person_id", "persons")
    dup_trips = report_duplicates(df_trips, "egt_trip_id", "trips")
    ...
    print(df_households[["household_id"]].value_counts()[:10])
    ...
    print(df_persons[["person_id"]].value_counts()[:10])
    ...
    print(df_trips[["trip_id"]].value_counts()[:10])
